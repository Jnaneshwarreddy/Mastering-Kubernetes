...
```bash
openssl genrsa -out jnaneshwar.key 2048
openssl req -new -key jnaneshwar.key -out jnaneshwar.csr -subj "/CN=jnaneshwar/O=clusteradmin"
openssl x509 -req -in jnaneshwar.csr -CA ca.crt -CAkey ca.key -CAcreateserial -out jnaneshwar.crt -days 365
```
